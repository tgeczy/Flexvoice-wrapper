...
static bool applyPitchToSpeakerLocked(WrapState* st, int pitchPercent) {
	...
	// (We’re calling this before speakRequest in the worker, so "now" doesn’t really matter,
	//  but true is fine and matches our earlier behavior.)
	try { st->engine->setSpeaker(sp, true); } catch (...) {}
		...
		try {
			{
				...
				for (const auto& p : cmd.parts) {
					...
					if (p.kind == Part::TEXT) {
						if (!p.text.empty()) {
							// Final safety: normalize again right before the engine sees it.
							std::string safe = normalizeFragileTokens(p.text);
							if (!safe.empty()) {
								st->engine->addFragment(safe.c_str());
							}
						}
					} else {
						st->engine->addBookmark(new FvwrapIndexBookmark(gen, p.index));
					}
				}
